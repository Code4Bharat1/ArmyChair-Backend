...
import { logActivity } from "../utils/logActivity.js";
  ...
  try {
    ...
    // ✅ Derive locationType from location string
    const destLocationType = toLocation.startsWith("PROD_")
      ? "PRODUCTION"
    ...
    const destQuery =
      source.type === "SPARE"
        ? { type: "SPARE", partName: source.partName, location: toLocation }
        : { type: "FULL", chairType: source.chairType, vendor: source.vendor, location: toLocation };

    // ✅ Build $setOnInsert without any undefined values
    const setOnInsert = {
      locationType: destLocationType,
      maxQuantity: source.maxQuantity ?? 0,
    ...
    if (source.type === "SPARE") {
      setOnInsert.partName = source.partName;
      setOnInsert.type = "SPARE";
    } else {
      setOnInsert.chairType = source.chairType;
      setOnInsert.colour = source.colour || "";
      setOnInsert.mesh = source.mesh || "";
      setOnInsert.vendor = source.vendor;
      setOnInsert.type = "FULL";
    }

    setOnInsert.location = toLocation; // ✅ Always include

    /* ===== UPDATE SOURCE ===== */
    source.quantity -= qty;
    await source.save();

    /* ===== UPDATE OR CREATE DEST ===== */
    await Inventory.findOneAndUpdate(
      destQuery,
      {
        $inc: { quantity: qty },
        $setOnInsert: setOnInsert,  // ✅ No undefined values
      },
      { upsert: true, new: true }   // ✅ Removed runValidators (causes issues with upsert + partial docs)
    );
